...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ПредметСтрокой = Центр_КалендарьСервер.ПолучитьПредставлениеПредмета(Объект.Предмет);
	//ТекущийПользователь = ПользователиКлиентСервер.ТекущийПользователь();
	//
	//ОтображатьПолеСотрудник = Центр_КалендарьСервер.ПолучитьПерсональнуюНастройку(
	...
	//
	//ОтображатьВремяС = Центр_КалендарьСервер.ПолучитьПерсональнуюНастройку("ОтображатьВремяС");
	//
	//ДоступноИзменениеКалендаря = Центр_КалендарьСервер.ДоступноИзменениеКалендаря(
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_ЗаписьКалендаря" Тогда
		...
		Если (ТипЗнч(Параметр) = Тип("СправочникСсылка.Центр_Календарь")
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//Центр_КалендарьСервер.УстановитьЦветЗаписиКалендаря(ТекущийОбъект.Ссылка, Цвет);
	//Центр_КалендарьСервер.УстановитьОтработанаЗаписьКалендаря(ТекущийОбъект.Ссылка, Отработана);
	//ОбработатьИзменениеНапоминанияНаСервере();
	
	Центр_КалендарьСервер.ПослеЗаписиСобытияКалендаря(ТекущийОбъект.Ссылка);
...
Процедура ПовторятьПриИзменении(Элемент)
	...
	Центр_КалендарьКлиент.ОткрытьФормуНастройкиПовторения(Объект.Ссылка);
...
Процедура ДатаНачалаВремяАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		Центр_КалендарьКлиент.СформироватьДанныеВыбораВремени(Текст, ДанныеВыбора, Объект.ДатаНачала);
	КонецЕсли;
...
Процедура ДатаОкончанияВремяАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		СтандартнаяОбработка = Ложь;
		Центр_КалендарьКлиент.СформироватьДанныеВыбораВремени(Текст, ДанныеВыбора, Объект.ДатаОкончания);
	КонецЕсли;
...
Процедура ВесьДеньПриИзменении(Элемент)
	...
	Центр_КалендарьКлиент.ПриИзмененииВесьДень(
...
Процедура СотрудникПриИзменении(Элемент)
	...
	//
	//ДоступноИзменениеКалендаря = Центр_КалендарьСервер.ДоступноИзменениеКалендаря(ТекущийПользователь, Объект.Сотрудник);
...
Процедура СотрудникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Центр_КалендарьКлиент.ВыбратьСотрудникаИзАдреснойКниги(Элемент, Объект.Сотрудник);
...
Процедура СотрудникОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	Центр_КалендарьКлиент.СотрудникОбработкаВыбора(Объект, "Сотрудник", ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура УстановитьПометкаУдаления(Команда)
	...
	Центр_КалендарьКлиент.УстановитьПометкуУдаления(Объект.Ссылка, НЕ Объект.ПометкаУдаления);
...
Процедура ОбновитьПредставлениеМестногоЧасовогоПояса()
	...
	НастройкаРежимОтображенияМестногоВремени = Центр_КалендарьСервер.НастройкаРежимОтображенияМестногоВремени();
	Если НастройкаРежимОтображенияМестногоВремени <> Перечисления.РежимыОтображенияМестногоВремени.ВремяСеанса Тогда
		
		ЧасовойПоясПользователя = Центр_КалендарьСервер.ЧасовойПоясПользователя(
			Сотрудники.ПользовательСотрудника(Объект.Сотрудник));
		
		ЧасовойПоясПоУмолчанию = Центр_КалендарьСервер.ЧасовойПоясПоУмолчанию();
		Если ЧасовойПоясПользователя <> ЧасовойПоясПоУмолчанию Тогда
			ПредставлениеМестногоЧасовогоПояса =
				Центр_КалендарьСервер.ПредставлениеЧасовогоПоясаДО(ЧасовойПоясПользователя);
		Иначе
			ПредставлениеМестногоЧасовогоПояса = "";
		КонецЕсли;
...
Процедура УстановитьПодписьПовторять()
	...
	//	ПовторениеСтрокой =
	//		Центр_КалендарьКлиентСервер.ПолучитьТекстовоеПредставлениеПовторения(НастройкиПовторения);
	//КонецЕсли;
...
Процедура ВывестиДлительностьСобытия()
	...
	ПодписьДней = Центр_КаленарьКлиентСервер.ПолучитьПодписьДней(Дней);
	
	Часов = Цел((ДлительностьСек - Дней * 86400) / 3600); // 86400 - число секунд в сутках
	ПодписьЧасов = Центр_КаленарьКлиентСервер.ПолучитьПодписьЧасов(Часов);
	
	Минут = Цел((ДлительностьСек - Дней * 86400 - Часов * 3600) / 60); // 86400 - число секунд в сутках
	ПодписьМинут = Центр_КаленарьКлиентСервер.ПолучитьПодписьМинут(Минут);
...
Процедура ЗаполнитьСпискиВыбора()
	...
	Если СобытиеВПределахОдногоДня Тогда
		...
		ТекДата = Центр_КаленарьКлиентСервер.КонецПолучаса(Объект.ДатаНачала);
...
Процедура ПрочитатьЦвет()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка)
		...
		Цвет = Центр_КалендарьСервер.ПолучитьЦветСобытияКалендаря(Параметры.ЗначениеКопирования);
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка)
		...
		Цвет = Центр_КалендарьСервер.ПолучитьЦветСобытияКалендаря(Параметры.Основание.ПовторяющеесяСобытие);
	Иначе
		//Цвет = Центр_КалендарьСервер.ПолучитьЦветСобытияКалендаря(Объект.Ссылка);
	КонецЕсли;
...
Процедура ПриИзмененииЦвета()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Центр_КалендарьСервер.УстановитьЦветЗаписиКалендаря(Объект.Ссылка, Цвет);
	КонецЕсли;
...
Процедура ПриИзмененииОтработана()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Центр_КалендарьСервер.УстановитьОтработанаЗаписьКалендаря(Объект.Ссылка, Отработана);
	КонецЕсли;
	ПриИзмененииЦвета();
...
Процедура ПрочитатьОтработана()
	...
	Отработана = Центр_КалендарьСервер.ЗаписьКалендаряОтработана(Объект.Ссылка);
...
Процедура ИнициализироватьКалендарь()
	...
	ЧасовойПоясТекущегоПользователя =
		Центр_КалендарьСервер.ЧасовойПоясПользователя(Пользователи.ТекущийПользователь());
	...
	Иначе
		ПоказыватьКалендарь = Центр_КалендарьСервер.ПолучитьПерсональнуюНастройку(
...
Процедура ОбновитьОтображениеТекущейЗаписиВКалендаре()
	...
	ИначеЕсли ПериодОтображения = ПредопределенноеЗначение("Перечисление.ПериодОтображенияРабочегоКалендаря.День") Тогда
		Центр_КаленарьКлиентСервер.ОбновитьТекущуюЗапись(Планировщик, НастройкиОтображения);
	КонецЕсли;
...
Процедура ТипЗаписиКалендаряПриИзмененииСервер()
	Элементы.ГруппаЗадача.Видимость = Объект.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.Задача;
	КонецПроцедуры
