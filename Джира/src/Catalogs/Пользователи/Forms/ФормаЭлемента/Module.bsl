...
&НаКлиенте
Процедура Центр_ПользовательскиеНастройки(Команда)
	ПараметрыФормы = Новый Структура("ВыбранныйПользователь", Объект.Ссылка);
	ОткрытьФорму("Обработка.Центр_РедактированиеПользовательскихНастроек.Форма.Главная", ПараметрыФормы, ,Новый УникальныйИдентификатор); 
КонецПроцедуры
#КонецОбласти
