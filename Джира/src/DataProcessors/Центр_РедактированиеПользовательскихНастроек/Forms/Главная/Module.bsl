...
&НаСервере
Функция НастройкаДереваПоЗначению(ЗначениеНастройки, ПолеНастройки)
	ДеревоНаст = РеквизитФОрмыВЗначение("ДеревоНастроек");
	ЗначениеНстройки = ДеревоНаст.Строки.Найти(ЗначениеНастройки, "Настройка", Истина).Значение;
	
	Если ЗначениеЗаполнено(ЗначениеНстройки) Тогда
		Возврат ЗначениеНстройки[ПолеНастройки];
	Иначе
