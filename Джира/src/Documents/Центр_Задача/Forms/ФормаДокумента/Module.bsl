...
Процедура ВыбратьГлавнуюЗадачу(Команда)
	...
	ОткрытьФорму("Документ.Центр_Задача.ФормаВыбора", , , , , , ОписаниеОповещенияОЗакрытииФормыВыбора);
КонецПроцедуры
...
Процедура СоздатьПодчиненнуюЗадачу(Команда)
	ПараметрыФормы = Новый Структура("ГлавнаяЗадача", Объект.Ссылка);
...
Процедура СформироватьДеревоДокументов()
	...
	НоваяСтрока.Представление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КореньЦепочки, "Представление");
...
Процедура ВывестиПодчиненныеОбъекты(ОбъектНачала, ДеревоПодчиненных)
	...
	Запрос.Текст =
		...
		|	ПРЕДСТАВЛЕНИЕ(Центр_Задача.Ссылка) КАК Представление
		|ИЗ
