...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) И ПараметрИсточникПараметров = Документы.Центр_Задача.ПустаяСсылка() Тогда
		Центр_ОбработчикиФормыЗадачиСервер.ЗаполнитьПользовательскиеНастройки(ЭтотОбъект);
	КонецЕсли;
	
	Если (НЕ Центр_ОбработчикиФормыЗадачиСервер.ЕстьПользовательскиеНастройки() И НЕ ЗначениеЗаполнено(Объект.Ссылка)) ИЛИ НЕ ЗначениеЗаполнено(Объект.Метка) Тогда
...
Процедура ОткрытьВопросы(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаСпискаВопросовПоЗадаче", ПараметрыФормы, , , , ,Оповещение
...
Процедура ОтклонитьЗадачу(Команда) Экспорт
	...
	ОткрытьФорму("Документ.Центр_Задача.Форма.ФормаОтклоненияЗадачи", ПараметрыФормы,,,,, Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура СоздатьГлавнуюЗадачу(Команда)
	Если Центр_ОбработчикиФормыЗадачиСервер.ЕстьПользовательскиеНастройки() Тогда
...
Процедура СоздатьПодчиненнуюЗадачу(Команда)
	...
	Иначе
		...
		Иначе
			...
			ОткрытьФорму("Документ.Центр_Задача.Форма.ФормаДокумента", ПараметрыФормы, , , , , НовыйОповещение);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗадачаВКалендарь(Команда)
	Центр_КалендарьКлиент.СоздатьЗаписьКалендаряЗадача(Объект.Ссылка,Исполнитель);
КонецПроцедуры
