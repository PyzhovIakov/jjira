...
Процедура ОтклонитьЗадачуНаСервере()
	...
	Адресат = ?(ТипЗнч(Объект.Постановщик) = Тип("СправочникСсылка.Пользователи"), Объект.Постановщик, Объект.ГлавныйМенеджер);
	Центр_УведомленияПользователей.ОтправитьУведомлениеПользователю(Адресат, ТекстСообщение, ТекущаяДатаСеанса(),
