...
Функция СтруктурыЭлементовЗаписи(ЗаписьКалендаря,
	...
	Если ТипЗнч(ЗаписьКалендаря) = Тип("СтрокаТаблицыЗначений") Тогда
		...
		//ФизЛицо = ЗаписьКалендаря.Сотрудник; 
	...
	ЭлементПланировщика.Значение =
		Центр_КаленарьКлиентСервер.СформироватьЭлементЗаписиКалендаря(ЗаписьКалендаря);
...
Функция ПолучитьЦветТабличногоДокумента(ТипЗаписи) Экспорт  
	...
	ТипЦвета = Справочники.Центр_КалендарьЦвета.НайтиПоРеквизиту("ТипЗаписиКалендаря",ТипЗаписи).Цвет;	
	Если ТипЦвета = Перечисления.ЦветаРабочегоКалендаря.ПустаяСсылка() Тогда
		Возврат ЦветаСтиля.ЦветКалендаряПоУмолчанию;
	КонецЕсли;
    
		
	Если ТипЦвета = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Красный") Тогда
		ЦветТабличногоДокумента = ЦветаСтиля.ЦветКалендаряКрасный;
	ИначеЕсли ТипЦвета = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Синий") Тогда
		ЦветТабличногоДокумента = ЦветаСтиля.ЦветКалендаряСиний;
	ИначеЕсли ТипЦвета = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Желтый") Тогда
		ЦветТабличногоДокумента = ЦветаСтиля.ЦветКалендаряЖелтый;
	ИначеЕсли ТипЦвета = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Зеленый") Тогда     
		ЦветТабличногоДокумента = ЦветаСтиля.ЦветКалендаряЗеленый;
	ИначеЕсли ТипЦвета = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Оранжевый") Тогда
